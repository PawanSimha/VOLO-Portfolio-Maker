...
<body class="bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 min-h-screen {{#if (eq themeMode 'dark')}}dark{{/if}}">
  ...
  <header class="relative overflow-hidden bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white py-24 md:py-32 lg:py-40 px-6 min-h-[600px] flex items-center transition-colors">
    <div class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-[0.04] dark:opacity-10"></div>
    <div class="relative max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center w-full">
      <div class="text-center md:text-left space-y-6 order-2 md:order-1">
        <div class="space-y-4">
          {{#if headerTitle}}
          <div class="inline-block px-4 py-1.5 bg-{{themeColor}}-500/10 text-{{themeColor}}-600 dark:text-white text-sm font-bold rounded-lg uppercase tracking-widest border border-{{themeColor}}-500/20">{{headerTitle}}</div>
          {{/if}}
          <h1 class="text-4xl md:text-7xl font-extrabold tracking-tight leading-tight">{{name}}</h1>
          <p class="text-slate-600 dark:text-slate-300 text-base md:text-xl leading-relaxed max-w-xl mx-auto md:mx-0 break-words">{{bio}}</p>
        </div>
        ...
        <div class="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
          ...
          {{#if linkedin}}
          <a href="{{linkedin}}" target="_blank" class="px-8 py-4 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-white rounded-2xl font-bold hover:bg-slate-300 dark:hover:bg-slate-600 transition-all border border-slate-200 dark:border-slate-600 flex items-center gap-2">
            LinkedIn
        ...
        <div class="flex flex-wrap justify-center md:justify-start gap-8 md:gap-10 pt-10 border-t border-slate-200 dark:border-slate-800/50">
          {{#if stats.projects}}
          <div>
            <p class="text-3xl font-extrabold text-slate-900 dark:text-white">{{stats.projects}}</p>
            <p class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Projects Done</p>
          </div>
          {{/if}}
          {{#if stats.contributions}}
          <div>
            <p class="text-3xl font-extrabold text-slate-900 dark:text-white">{{stats.contributions}}</p>
            <p class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">GitHub Contrib.</p>
          </div>
          {{/if}}
          {{#if stats.satisfaction}}
          <div>
            <p class="text-3xl font-extrabold text-slate-900 dark:text-white">{{stats.satisfaction}}</p>
            <p class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Client Satisfaction</p>
          </div>
          {{/if}}
        </div>
        {{/if}}
      ...
      <div class="order-1 md:order-2 flex justify-center md:justify-end">
        ...
        <div class="relative">
          ...
          <img src="{{image}}" class="w-64 h-64 md:w-96 md:h-96 rounded-[3rem] object-cover border-8 border-slate-200 dark:border-slate-800 shadow-2xl relative z-10">
  ...
  <main class="w-full px-8 md:px-16 lg:px-24 py-16 md:py-24 space-y-24 bg-slate-50 dark:bg-slate-950">
        ...
        <div class="project-card group p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm hover:shadow-xl dark:hover:shadow-{{themeColor}}-500/10 flex flex-col h-full overflow-hidden text-left {{#if (gt @index 1)}}hidden{{/if}}">
          <div class="mb-4">
             <div class="flex justify-between items-start mb-1">
               <span class="inline-block px-3 py-1 bg-{{themeColor}}-50 dark:bg-{{themeColor}}-950/30 text-{{themeColor}}-600 dark:text-white text-xs font-bold rounded-full uppercase tracking-wider">Project</span>
          ...
          <button onclick="toggleReadMore(this)" class="text-{{themeColor}}-600 dark:text-white font-semibold text-sm hover:underline text-left">Read More</button>
          {{/if}}
        </div>
        {{/each}}
      </div>
      {{#if (gt projects.length 2)}}
      <button onclick="showAllProjects(this)" class="mt-10 px-8 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-{{themeColor}}-600 dark:text-white font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm active:scale-95">Show All Projects</button>
      {{/if}}
    </section>
    {{/if}}
    {{#if education.length}}
    <section id="education" class="text-center">
      ...
      <div class="grid md:grid-cols-2 gap-8">
        ...
        <div class="edu-item p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm text-left {{#if (gt @index 3)}}hidden{{/if}}">
          <div class="flex justify-between items-start mb-2">
            ...
            <span class="text-sm font-bold text-{{themeColor}}-600 dark:text-white bg-{{themeColor}}-50 dark:bg-{{themeColor}}-950/30 px-3 py-1 rounded-full">{{formatDate year}}</span>
          </div>
      ...
      <button onclick="showAllEducation(this)" class="mt-10 px-8 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-{{themeColor}}-600 dark:text-white font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm active:scale-95">Show More Education</button>
      {{/if}}
    </section>
    {{/if}}
    ...
    <section id="experience" class="text-center">
      ...
      <div class="space-y-8">
        ...
        <div class="experience-item p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm text-left flex flex-col md:flex-row gap-6 {{#if (gt @index 2)}}hidden{{/if}}">
          <div class="md:w-1/4 flex flex-col gap-4">
            <span class="text-sm font-bold text-{{themeColor}}-600 dark:text-white bg-{{themeColor}}-50 dark:bg-{{themeColor}}-950/30 px-4 py-2 rounded-xl inline-block w-fit text-center">{{formatDate date}}</span>
            {{#if image}}
          ...
          <div class="md:w-3/4">
            ...
            {{#if (longDesc description 150)}}
            <button onclick="toggleReadMore(this)" class="text-{{themeColor}}-600 dark:text-white font-semibold text-sm hover:underline mt-2">Read More</button>
            {{/if}}
          </div>
        </div>
        {{/each}}
      ...
      <button onclick="showAllExperience(this)" class="mt-10 px-8 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-{{themeColor}}-600 dark:text-white font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm active:scale-95">Show More Experience</button>
      {{/if}}
    </section>
    {{/if}}
      ...
      <div id="awardsGrid" class="grid md:grid-cols-2 gap-8">
        ...
        <div class="award-item p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm text-left flex flex-col h-full {{#if (gt @index 1)}}hidden{{/if}}">
          ...
          <div class="flex justify-between items-start mb-2">
            ...
            <span class="text-sm font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-3 py-1 rounded-full shrink-0">{{formatDate date}}</span>
          </div>
          <p class="text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-3">{{description}}</p>
          {{#if (longDesc description 150)}}
          <button onclick="toggleReadMore(this)" class="text-{{themeColor}}-600 dark:text-white font-semibold text-sm hover:underline mt-2">Read More</button>
    ...
    <section id="certs" class="text-center">
        ...
        <div class="cert-item group bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all {{#if (gt @index 3)}}hidden{{/if}}">
          ...
          <div class="p-4 text-left border-t border-slate-50 dark:border-slate-800">
            <div class="flex justify-between items-start mb-1">
              ...
              <span class="text-[9px] font-bold text-{{themeColor}}-500 dark:text-white ml-2 shrink-0">{{formatDate date}}</span>
            </div>
      ...
      <button onclick="showAllCerts(this)" class="mt-10 px-8 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-{{themeColor}}-600 dark:text-white font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm active:scale-95">Show All Certificates</button>
      {{/if}}
    </section>
    {{/if}}

    <section id="contact" class="text-center py-0 bg-white dark:bg-slate-900 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
      <div class="py-16 md:py-20 px-8 space-y-6">
      <div class="inline-block px-4 py-1.5 bg-{{themeColor}}-50 dark:bg-{{themeColor}}-950/30 text-{{themeColor}}-600 dark:text-white text-sm font-bold rounded-full mb-6 uppercase tracking-widest">Get In Touch</div>
      ...
      <div class="flex flex-col md:flex-row justify-center gap-6 max-w-2xl mx-auto px-6">
        ...
        <a href="mailto:{{email}}" class="flex-1 flex items-center gap-4 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700 hover:border-{{themeColor}}-500 transition-all group shadow-sm">
          ...
          <div class="text-left">
            <p class="text-xs font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Email Me</p>
        ...
        <a href="https://wa.me/{{whatsapp}}" target="_blank" class="flex-1 flex items-center gap-4 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700 hover:border-green-500 transition-all group shadow-sm">
          <div class="w-14 h-14 bg-green-500 dark:bg-green-600 text-white rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-lg shadow-green-200 dark:shadow-none p-3">
          ...
          <div class="text-left">
            <p class="text-xs font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">WhatsApp Me</p>
  ...
  <footer class="bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 py-20 px-6 border-t border-slate-200 dark:border-slate-800 transition-colors">
    <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
      <div class="md:col-span-1 space-y-6">
        <div class="flex items-center gap-3">
          ...
          <span class="text-xl font-bold text-slate-900 dark:text-white">{{name}}</span>
        </div>
        <p class="text-sm leading-relaxed text-slate-600 dark:text-slate-400">A professional portfolio built with passion. Showcasing expertise, projects, and achievements with high-impact design.</p>
        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-green-500/10 dark:bg-green-500/20 text-green-400 dark:text-green-300 rounded-lg text-xs font-bold border border-green-500/20 dark:border-green-500/30">
      ...
      <div>
        <h3 class="text-slate-900 dark:text-white font-bold mb-6 uppercase tracking-widest text-[10px]">Quick Navigation</h3>
        <ul class="space-y-4 text-sm">
          {{#if aboutMe}}<li><a href="#about" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">About Me</a></li>{{/if}}
          {{#if projects.length}}<li><a href="#projects" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">Featured Projects</a></li>{{/if}}
          {{#if (or skills.length softSkills.length)}}<li><a href="#skills" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">Skills & Expertise</a></li>{{/if}}
          <li><a href="#contact" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">Contact Me</a></li>
        </ul>
      </div>

      <div>
        <h3 class="text-slate-900 dark:text-white font-bold mb-6 uppercase tracking-widest text-[10px]">Follow Me</h3>
        <ul class="space-y-4 text-sm">
          {{#if linkedin}}<li><a href="{{linkedin}}" target="_blank" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">LinkedIn</a></li>{{/if}}
          {{#if github}}<li><a href="{{github}}" target="_blank" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">GitHub</a></li>{{/if}}
          {{#if instagram}}<li><a href="{{instagram}}" target="_blank" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">Instagram</a></li>{{/if}}
          {{#if twitter}}<li><a href="{{twitter}}" target="_blank" class="text-slate-600 dark:text-slate-400 hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors">X (Twitter)</a></li>{{/if}}
        </ul>
      </div>

      <div class="space-y-6">
        <h3 class="text-slate-900 dark:text-white font-bold mb-6 uppercase tracking-widest text-[10px]">Let's Connect</h3>
        {{#if email}}
        <p class="text-sm text-slate-600 dark:text-slate-400">Have a question or proposal? Drop me a line anytime.</p>
        <a href="mailto:{{email}}" class="text-slate-900 dark:text-white font-bold hover:text-{{themeColor}}-600 dark:hover:text-{{themeColor}}-400 transition-colors flex items-center gap-2 group">
          {{email}}
        ...
        {{/if}}
        {{#if whatsapp}}
        <a href="https://wa.me/{{whatsapp}}" target="_blank" class="inline-block mt-4 px-4 py-2 bg-green-500/10 dark:bg-green-600 text-green-400 dark:text-white rounded-lg text-xs font-bold border border-green-500/20 dark:border-green-600/30 hover:bg-green-500 hover:text-white transition-all">
    ...
    <div class="max-w-6xl mx-auto mt-20 pt-8 border-t border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-medium tracking-wider uppercase text-slate-500">
...
function render() {
  ...
  // Apply dark mode BEFORE injecting HTML so Tailwind processes dark: classes
